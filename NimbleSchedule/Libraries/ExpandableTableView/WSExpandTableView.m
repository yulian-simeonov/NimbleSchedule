...
//  WSExpandTableView
//
//  Created by Yurii Bogdan on 25/11/2013
//  Copyright (c) 2013 Yurii Bogdan. All rights reserved.
//

#import "WSExpandTableView.h"

@implementation WSExpandTableView
@synthesize WSExpandTableViewDelegate, WSExpandTableViewDataSource;
...
{
	return [WSExpandTableViewDataSource tableView:tableView numberOfRowsInSection:section];
}

-(NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
	return [WSExpandTableViewDataSource numberOfSectionsInTableView:tableView];
}

- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section
{
	if ([WSExpandTableViewDataSource respondsToSelector:@selector(tableView:titleForHeaderInSection:)])
	return [WSExpandTableViewDataSource tableView:tableView titleForHeaderInSection:section];
		return nil;
}

- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section
{	if ([WSExpandTableViewDataSource respondsToSelector:@selector(tableView:titleForFooterInSection:)])
	return [WSExpandTableViewDataSource tableView:tableView titleForFooterInSection:section];
		return nil;
}

- (BOOL)tableView:(UITableView *)tableView canEditRowAtIndexPath:(NSIndexPath *)indexPath
{	if ([WSExpandTableViewDataSource respondsToSelector:@selector(tableView:canEditRowAtIndexPath:)])
	return [WSExpandTableViewDataSource tableView:tableView canEditRowAtIndexPath:indexPath];
	return NO;
}

- (BOOL)tableView:(UITableView *)tableView canMoveRowAtIndexPath:(NSIndexPath *)indexPath
{
	if ([WSExpandTableViewDataSource respondsToSelector:@selector(tableView:canMoveRowAtIndexPath:)])
	return [WSExpandTableViewDataSource tableView:tableView canMoveRowAtIndexPath:indexPath];
	return NO;
}

- (NSArray *)sectionIndexTitlesForTableView:(UITableView *)tableView
{
	if ([WSExpandTableViewDataSource respondsToSelector:@selector(sectionIndexTitlesForTableView:)])
	return [WSExpandTableViewDataSource sectionIndexTitlesForTableView:tableView];
	return nil;
}

- (NSInteger)tableView:(UITableView *)tableView sectionForSectionIndexTitle:(NSString *)title atIndex:(NSInteger)index
{
	if ([WSExpandTableViewDataSource respondsToSelector:@selector(tableView:sectionForSectionIndexTitle:atIndex:)])
	return [WSExpandTableViewDataSource tableView:tableView sectionForSectionIndexTitle:title atIndex:index];
	return 0;
}

- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
	if ([WSExpandTableViewDataSource respondsToSelector:@selector(tableView:commitEditingStyle:forRowAtIndexPath:)])
	return [WSExpandTableViewDataSource tableView:tableView commitEditingStyle:editingStyle forRowAtIndexPath:indexPath];
}

- (void)tableView:(UITableView *)tableView moveRowAtIndexPath:(NSIndexPath *)sourceIndexPath toIndexPath:(NSIndexPath *)destinationIndexPath
{
	if ([WSExpandTableViewDataSource respondsToSelector:@selector(tableView:moveRowAtIndexPath:toIndexPath:)])
	return [WSExpandTableViewDataSource tableView:tableView moveRowAtIndexPath:sourceIndexPath toIndexPath:destinationIndexPath];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (self.expandOnlyOneCell)
	{
		if (actionToTake == 0) // e.g. the first time or an expanded cell from before gets in to view
		{
			if (selectedIndexPath)
				...
				{
					cell = [WSExpandTableViewDataSource tableView:tableView cellForRowAtIndexPath:indexPath isExpanded:YES];//i want it expanded
					return cell;
				}
			
			cell = [WSExpandTableViewDataSource tableView:tableView cellForRowAtIndexPath:indexPath isExpanded:NO];
		...
		if(actionToTake == -1)
		{
			[WSExpandTableViewDataSource tableView:tableView collapseCell:cell withIndexPath:indexPath];
			actionToTake = 0;
		}
		else
		{
			[WSExpandTableViewDataSource tableView:tableView expandCell:cell withIndexPath:indexPath];
			actionToTake = 0;
		}
	}
	else
	{
		if (actionToTake == 0) // e.g. the first time or an expanded cell from before gets in to view
		{
			...
			if (alreadyExpanded)
					cell = [WSExpandTableViewDataSource tableView:tableView cellForRowAtIndexPath:indexPath isExpanded:YES];
			else
				cell = [WSExpandTableViewDataSource tableView:tableView cellForRowAtIndexPath:indexPath isExpanded:NO];
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section
{
    if ([WSExpandTableViewDelegate respondsToSelector:@selector(tableView:heightForFooterInSection:)])
        return [WSExpandTableViewDelegate tableView:tableView heightForFooterInSection:section];
    return 0;
}
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
	if (self.expandOnlyOneCell)
	{
		if (selectedIndexPath)
		if(selectedIndexPath.row == indexPath.row && selectedIndexPath.section == indexPath.section)
			return [WSExpandTableViewDelegate tableView:tableView heightForRowAtIndexPath:indexPath isExpanded:YES];
		
		return [WSExpandTableViewDelegate tableView:tableView heightForRowAtIndexPath:indexPath isExpanded:NO];
	}
	else
	{
		BOOL alreadyExpanded = NO;
