...
class ShiftDropRequestViewController: UIViewController, ScheduleViewControllerDelegate {
    ...
    func updateTableContent() {
        if (stepNo == 0) {
            sectionTitleArray = [LocalizeHelper.sharedInstance.localizedStringForKey("PICK_A_SHIFT_TO_DROP")]
            identifierArray = ["ShiftPickUpCell"]
            cellHeiArray = [60]            
        } else if (stepNo == 1) {
            sectionTitleArray = [LocalizeHelper.sharedInstance.localizedStringForKey("PICK_A_SHIFT_TO_DROP"), LocalizeHelper.sharedInstance.localizedStringForKey("ENTER_A_MESSAGE")]
